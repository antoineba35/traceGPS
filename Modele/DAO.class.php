...
class DAO
{
    ...
    // true si l'adresse $adrMail existe dans la table tracegps_utilisateurs
    // false sinon
    ...
    public function getLesUtilisateursAutorisant($idUtilisateur)
    {
        ...
        $txt_req = "Select id, pseudo, mdpSha1, adrMail, numTel, niveau, dateCreation, nbTraces, dateDerniereTrace";
        $txt_req .= " from tracegps_vue_utilisateurs";
        $txt_req .= " where id IN (select idAutorisant";
    ...
    public function getLesUtilisateursAutorises($idUtilisateur)
    {
        ...
        $txt_req = "Select id, pseudo, mdpSha1, adrMail, numTel, niveau, dateCreation, nbTraces, dateDerniereTrace";
        $txt_req .= " from tracegps_vue_utilisateurs";
        $txt_req .= " where id IN (select idAutorise";
        $txt_req .= " from tracegps_autorisations";
    ...
    {
        if(DAO::autoriseAConsulter($idAutorisant, $idAutorise) == true)
        {
    ...
    public function creerUnPointDeTrace($unPointDeTrace)
    {
        ...
        $req->bindValue("id", utf8_decode($unPointDeTrace->getId()), PDO::PARAM_STR);
        ...
        if(utf8_decode($unPointDeTrace->getId()) == 1)
        {
            $txt = "SELECT `dateHeure` FROM `tracegps_points` WHERE id = :id";
            $req2 = $this->cnx->prepare($txt);
            $req2->bindValue("id", utf8_decode($unPointDeTrace->getId()), PDO::PARAM_STR);
            $req2->execute();
            $uneLigne = $req2->fetch(PDO::FETCH_OBJ);
            
            $req->bindValue("dateHeure", utf8_decode($uneLigne->dateHeure), PDO::PARAM_STR);
        }
        else
        ...
        $requete = "SELECT `idTrace`, `id`, `latitude`, `longitude`, `altitude`, `dateHeure`, `rythmeCardio` FROM `tracegps_points` WHERE `idTrace` = :id";
        $req3 = $this->cnx->prepare($requete);
        
        $req3->bindValue("id", utf8_decode($unPointDeTrace->getId()), PDO::PARAM_STR);
        $req3->execute();
        $uneLigne = $req3->fetchColumn(0);
        if($uneLigne == 0)
            return false;
        else
            return true;
